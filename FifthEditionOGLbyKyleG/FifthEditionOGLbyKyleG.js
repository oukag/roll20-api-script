...
var FifthEditionOGLbyKyleG = FifthEditionOGLbyKyleG || (function(){
        ...
        var version = '0.1.1',
            ...
            handleAmmo = function(msg) {
                log("handleAmmo");
                if(!EVENT_HANDLERS.AmmoTracking.trigger(msg) || msg.content.indexOf("ammo= charname=") !== -1) {
                    // UNABLE TO FIND AMMO
                    return;
                }
            ...
            handleSpellSlot = function(msg) {
                if(!EVENT_HANDLERS.SpellTracking.trigger(msg)) { return; }
                log("handleSpellSlot");
                log(msg);
            ...
            deathSaveEventHandler = function(msg) {
                if(EVENT_HANDLERS.DeathSaveTracking.trigger(msg)) {
            ...
            hitDiceEventHandler = function(msg) {
                if(EVENT_HANDLERS.HitDiceTracking.trigger(msg)) {
            ...
            damageToken =  function(tokenId, damage) {
                ...
                if(isTokenConcentrating(token)) { concentrationCheck(character.id, value); }
            ...
            updateHealthStatus = function(token) {
                ...
                if(curr <= (max * TokenSpecifics.DEAD_RATIO)) {
                    token.set(StatusMarkers.DEAD,true);
                    token.set(StatusMarkers.PRONE,true);
                }
            ...
            handleModHealth = function(msg) {
                if(COMMANDS.ModHealth.trigger(msg)) {
                    ...
                    } else { General.sendChat(scriptName, "/w \"" + msg.who.replace(' (GM)','') + "\" " + COMMANDS.ModHealth.help); }
                }
            },
            ...
            handleToggleRage = function(msg) {
                if(COMMANDS.ToggleRage.trigger(msg)) {
                    ...
                    if(!msg.selected || msg.content.split(" ").length > 1) {
                        ...
                        if(split.length <= 1) { General.sendChat(scriptName, "/w \"" + msg.who.replace(' (GM)','') + "\" You must have a token selected to use !toggleRage"); }
            ...
            handleToggleConcentration = function(msg) {
                if(COMMANDS.ToggleConcentration.trigger(msg)) {
                    ...
                    if(!msg.selected) {
                        General.sendChat(scriptName, "/w \"" + msg.who.replace(' (GM)','') + "\" You must have a token selected to use !toggleConcentration");
                        return;
                    }
            ...
            concentrationCheck = function(charId, damage) {
                log('concentrationCheck('+charId+','+damage+')');
                var saveDC = "[[{10,floor("+damage+"/2)}kh1]]";
                var output;
                if(!isNPC(charId)) {
                    var war_caster_feat = General.findAttr(charId, SpecificAttributes.WAR_CASTER_FEAT);
                    output = General.getCharacter(charId).get('name') + " needs to make a Concentration (Constitution) " +
                        "saving throw for taking damage" + (war_caster_feat ? " with advantage for having the War Caster feat." : ".") +
                        "Save DC " + saveDC;
            ...
            healingPotionEventHandler = function(msg) {
                if(EVENT_HANDLERS.PotionTracking.trigger(msg)) {
                    var simple = RollTemplates.parse(msg);
                    log(simple);
                ...
                if(EVENT_HANDLERS.DamageListener.trigger(msg)) {
                    ...
                    var isMagicWeapon = function () {
                        ...
                        var hasMagicLabel = function (dmg) {
                            if (!dmg || !dmg.inlinerolls || !dmg.inlinerolls.results || !dmg.inlinerolls.results.rolls) {
                                return false;
                            }
                            var result = false;
                            _.each(dmg.inlinerolls.results.rolls, function (roll) {
                                if (roll.text && roll.text.toLowerCase().indexOf("magic") !== -1) { result = true; }
                            });
                            return result;
                        };
                    ...
                    if (tpl && tpl.dmg2flag && tpl.dmg2type) {
                        ...
                        if (saveForHalf) { btns.push(createButton(strDmg2, "save2", Math.floor(tpl.dmg2.total * 0.5))); }
        ...
        var COMMANDS = {
            LongRest: { name:'longrest', handler: handleLongRest,
                ...
                "as well as handle the character's hit points, hit dice, spell slots, exhaustion level, and will clear " +
                "a character's inspiration and inspiration value attributes (<b>" + SpecificAttributes.INSPIRATION + ", " +
                SpecificAttributes.INSPIRATION_VALUE + "</b>) if it was from a bard's inspiration.</p>" +
                "<p><b>--charids character_id1, character_id2</b> OPTIONAL - performs the short rest for the characters " +
                "with the given character ids.</p>" +
                "<p><b>NOTE:</b> if there are selected tokens, only those tokens will get a long rest even if you have " +
            ...
            ShortRest: { name:'shortrest', handler: handleShortRest,
                ...
                "rest abilities attribute (<b>" + SpecificAttributes.SHORT_REST_ABILITIES + "</b>). These abilities must " +
                "be specified as a character ability in the Attributes and Abilities tab of the character sheet.</p>" +
                "<p><b>--charids character_id1, character_id2</b> OPTIONAL - performs the short rest for the characters " +
                "with the given character ids.</p>" +
                "<p><b>NOTE:</b> if there are selected tokens, only those tokens will get a short rest even if you have " +
                "provided character ids.</p>"
            },
            ...
            Inspire: { name:'inspire', handler: handleInspire,
                ...
                "<p><b>--charid|character_id</b> OPTIONAL - the character id of the bard inspiring the target. This is " +
                "necessary to decrement the number of bardic inspiration resources left for the character.</p>" +
                "<p><b>--value|value</b> OPTIONAL - the inspiration value for the character is set to this value.</p>" +
                "<p>If no charid or value is provided, it is assumed that it is gm inspiration to be given and sets " +
            ...
            UseInspiration: { name:'useInspiration', handler: handleUseInspiration,
                ...
                "<p>If the inspiration value is some sort of die roll, such as from a bard's inspiration, will roll the " +
        ...
        var EVENT_HANDLERS = {
            ...
            SpellTracking:       { name:"spelltracking",handler:handleSpellSlot,
                trigger: function(msg){
                    var tpls = [
            ...
            DamageListener:      { name:"damagelistener", handler:damageEventHandler,
                trigger: function(msg){
                    var tpls =[
                        RollTemplates.TEMPLATES.ATKDMG.type,
                        RollTemplates.TEMPLATES.DMG.type,
