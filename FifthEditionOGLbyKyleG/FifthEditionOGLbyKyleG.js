// 5th Edition OGL by Kyle G. Companion Script
// 
...
var FifthEditionOGLbyKyleG = FifthEditionOGLbyKyleG || (function(){
	...
	SpecificAttributes = {
		...
		SHOW_WOUNDS            : 'showWounds',			// Determines if the bloodied status should be applied to tokens
		// Death saving throws
	...
	DamageTypes = (function(){
		...
		isRageResistantToType = function(charId, keyword) {
			...
			if(bearTotemAttr && ['1', 'on', 'true'].indexOf(bearTotemAttr.get('current')) !== -1) { resist = type.bear; }
			else { resist = type.rage; }
			log('Resist: ' + resist);
			return resist;
		},
	...
	Resource = (function(){
		...
		obj.set = function(n,v) {
			...
			switch (n) {
				case 'name':
					obj.name = v;
					attr = General.findAttr(obj.characterid, obj.resource + SUFFIX.NAME);
					attr.set('current', v);
					break;
				case 'current':
				case 'max':
	...
	clearSpellSlotsForCharacter = function(charId) {
		...
		for(var spelllevel = 1; spelllevel <= 9; spelllevel++) {
			var remaining = General.findOrCreateAttr(charId, remainingAttrName.replace("SPELLLEVEL", spelllevel.toString()));
			var max = General.findAttr(charId, maxAttrName.replace("SPELLLEVEL", spelllevel.toString()));
	...
	handleDivinePortent = function(msg) {
		if(msg.type == "api" && msg.content.indexOf("!divinePortent") !== -1) {
			...
			if(!portent || !charid) {
				General.whisperError(scriptName, "Tried to use divine portent but received the following values<br>"
			...
			if(!parseInt(res.current)) {
				General.whisperError(scriptName, "Tried to use divine portent but portent value was '" + res.current + "'");
				return;
			}
			var output = RollTemplates.formatOutput({
				type: RollTemplates.TEMPLATES.SIMPLE.type,
	...
	updateHealthStatus = function(token) {
		...
		var showWoundsAttr = General.findAttr(character.id, SpecificAttributes.SHOW_WOUNDS);
		var showWounds = true;
		if(showWoundsAttr) {
	...
	handleToggleConcentration = function(msg) {
		if(msg.type == "api" && msg.content.indexOf("!toggleConcentration") !== -1) {
			...
			_.each(msg.selected, function(sel) { toggleTokenConcentration(General.getToken(sel._id)); });
		}
	},
	...
	concentrationCheck = function(charId, damage) {
		...
		if(!isNPC(charId)) {
			...
			output = General.getCharacter(charId).get('name') + " needs to make a Concentration (Constitution) saving throw for taking damage while concentrating"
	...
	damageEventHandler = function(msg) {
		...
		if(tpl.dmg1type) {
			...
			if(saveForHalf) { btns.push(createButton(strDmg1, "save1", Math.floor(tpl.dmg1.total * 0.5))); }

			if(tpl.crit1 && tpl.hldmg) { btns.push(createButton(strDmg1, "hlcrit", tpl.dmg1.total + tpl.hldmg.total + tpl.crit1.total)); }
			else if(tpl.crit1) { btns.push(createButton(strDmg1, "crit1", tpl.dmg1.total + tpl.crit1.total));}
			else if(tpl.hldmg) { btns.push(createButton(strDmg1, "hldmg", tpl.dmg1.total + tpl.hldmg.total));  }

			if(saveForHalf && tpl.crit1 && tpl.hldmg) { btns.push(createButton(strDmg1, "save1hlcrit", Math.floor((tpl.dmg1.total + tpl.hldmg.total + tpl.crit1.total) * 0.5))); }
		...
		if(tpl.dmg2type) {
			...
			if(tpl.dmg2flag) { btns.push(createButton(strDmg2, "dmg2", tpl.dmg1.total)); }
			if(saveForHalf) { btns.push(createButton(strDmg2, "save2", Math.floor(tpl.dmg1.total * 0.5))); }

			if(tpl.crit2) { btns.push(createButton(strDmg2, "crit2", tpl.dmg2.total + tpl.crit2.total));}
			if(saveForHalf && tpl.crit2) { btns.push(createButton(strDmg2, "save2crit", Math.floor((tpl.dmg2.total + tpl.crit2.total) * 0.5))); }
		}
	...
	handleStatus = function() {
		...
		formatStatusOutput = function(name, value) { return "<span style='display:block;'>"+name+": "+value+"</span>"; },

		statustrackingstatus    = getStatus(state.FifthEditionOGLbyKyleG.statustracking),
		ammotrackingstatus      = getStatus(state.FifthEditionOGLbyKyleG.ammotracking),
		spelltrackingstatus     = getStatus(state.FifthEditionOGLbyKyleG.spelltracking),
		damagetrackingstatus    = getStatus(state.FifthEditionOGLbyKyleG.damagetracking),
		deathsavetrackingstatus = getStatus(state.FifthEditionOGLbyKyleG.deathsavetracking),
		hitdicetrackingstatus   = getStatus(state.FifthEditionOGLbyKyleG.hitdicetracking),
		potiontrackingstatus    = getStatus(state.FifthEditionOGLbyKyleG.potiontracking),

		output = '<p>'
			+ formatStatusOutput('statustracking',      statustrackingstatus)
			+ formatStatusOutput('ammotracking',      ammotrackingstatus)
			+ formatStatusOutput('spelltracking',     spelltrackingstatus)
			+ formatStatusOutput('damagetracking',    damagetrackingstatus)
			+ formatStatusOutput('deathsavetracking', deathsavetrackingstatus)
			+ formatStatusOutput('hitdicetracking',   hitdicetrackingstatus)
			+ formatStatusOutput('potiontracking',    potiontrackingstatus)
			+ "<span style='display:block; margin-top:5px;'>" + scriptName + "</span>"
			+ "</p>";

		General.sendChat(scriptName, RollTemplates.formatOutput({
			type: RollTemplates.TEMPLATES.DESC.type,
			desc: output
		}));
	},
	...
	loadState = function() {
        ...
        if(!state.FifthEditionOGLbyKyleG.statustracking) {
            state.FifthEditionOGLbyKyleG.statustracking = 'on';
        }
        if(!state.FifthEditionOGLbyKyleG.ammotracking) {
            state.FifthEditionOGLbyKyleG.ammotracking = 'on';
        }
        if(!state.FifthEditionOGLbyKyleG.spelltracking) {
            state.FifthEditionOGLbyKyleG.spelltracking = 'on';
        }
        if(!state.FifthEditionOGLbyKyleG.damagetracking) {
            state.FifthEditionOGLbyKyleG.damagetracking = 'on';
        }
        if(!state.FifthEditionOGLbyKyleG.deathsavetracking) {
            state.FifthEditionOGLbyKyleG.deathsavetracking = 'on';
        }
        if(!state.FifthEditionOGLbyKyleG.hitdicetracking) {
            state.FifthEditionOGLbyKyleG.hitdicetracking = 'on';
        }
        if(!state.FifthEditionOGLbyKyleG.potiontracking) {
            state.FifthEditionOGLbyKyleG.potiontracking = 'on';
        }
	};

	var COMMANDS = {
        LongRest:            { name:'longrest',            handler: handleLongRest,            help:''},
        ShortRest:           { name:'shortrest',           handler: handleShortRest,           help:''},
        DivinePortent:       { name:'divinePortent',       handler: handleDivinePortent,       help:''},
        Inspire:             { name:'inspire',             handler: handleBardInspiration,     help:''},
        UseInspiration:      { name:'useInspiration',      handler: handleUseInspiration,      help:''},
        ToggleRage:          { name:'toggleRage',          handler: handleToggleRage,          help:''},
        ToggleConcentration: { name:'toggleConcentration', handler: handleToggleConcentration, help:''},
        ModHealth:           { name:'modHealth',           handler: handleModHealth,           help:''},
        UpdateDefaultToken:  { name:'updateDefaultToken',  handler: handleUpdateDefaultToken,  help:''},
        Status:              { name:'5estatus',            handler: handleStatus,              help:''}
	};

	var EVENT_HANDLERS = {
		AmmoTracking:      { handler:handleAmmo,
			trigger: function(msg){
				return msg.content.indexOf("ammo=") !== -1;
			},
			help: ''},
		SpellTracking:     { handler:handleSpellSlot,
			trigger: function(msg){
				return [RollTemplates.TEMPLATES.SPELL.type, RollTemplates.TEMPLATES.ATKDMG.type, RollTemplates.TEMPLATES.DMG.type].indexOf(msg.rolltemplate) !== -1;
			},
			help: ''},
		DamageTracking:    { handler:damageEventHandler,
			trigger: function(msg){
				return [RollTemplates.TEMPLATES.ATKDMG.type, RollTemplates.TEMPLATES.DMG.type, RollTemplates.TEMPLATES.NPCACTION.type, RollTemplates.TEMPLATES.NPCDMG.type].indexOf(msg.rolltemplate) !== -1;
			},
			help: ''},
		DeathSaveTracking: { handler:deathSaveEventHandler,
			trigger: function(msg){
				return [RollTemplates.TEMPLATES.SIMPLE.type].indexOf(msg.rolltemplate) !== -1 && msg.content.indexOf('^{death-save-u}') !== -1;
			},
			help: ''},
		HitDiceTracking:   { handler:hitDiceEventHandler,
            trigger: function(msg){
                return [RollTemplates.TEMPLATES.SIMPLE.type].indexOf(msg.rolltemplate) !== -1 && msg.content.indexOf('^{hit-dice-u}') !== -1;
            },
            help: ''},
		PotionTracking:    { handler:healingPotionEventHandler,
            trigger: function(msg){
                return [RollTemplates.TEMPLATES.SIMPLE.type].indexOf(msg.rolltemplate) !== -1 && msg.content.indexOf('POTION OF ') !== -1 && msg.content.indexOf('HEALING') !== -1;
            },
            help: ''},
	};
