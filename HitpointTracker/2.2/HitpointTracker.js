...
var HitpointTracker = HitpointTracker || (function(){
	...
	damageEventHandler = function(msg){
		...
		var str = "[Apply name amount](!modHealth --sel --damage amount type)";
		var dmg1  = (template.dmg1flag && template.dmg1type) ? str.replace("name", "dmg1").replace(/amount/g, template.dmg1.total).replace("type", template.dmg1type) : null,
			crit1 = (template.crit1    && template.dmg1type) ? str.replace("name", "crit1").replace(/amount/g, template.dmg1.total + template.crit1.total).replace("type", template.dmg1type) : null,
			dmg2  = (template.dmg2flag && template.dmg2type) ? str.replace("name", "dmg2").replace(/amount/g, template.dmg2.total).replace("type", template.dmg2type) : null,
			crit2 = (template.crit2    && template.dmg2type) ? str.replace("name", "crit2").replace(/amount/g, template.dmg2.total + template.crit2.total).replace("type", template.dmg2type) : null,
			hldmg = (template.hldmg    && template.dmg1type) ? str.replace("name", "hldmg").replace(/amount/g, template.dmg1.total + template.hldmg.total + (template.crit1 ? template.crit1.total : 0)).replace("type", template.dmg1type) : null,
			savedmg1 = (saveForHalf    && template.dmg1type) ? str.replace("name", "save").replace(/amount/g, Math.floor((template.dmg1.total + (template.hldmg ? template.hldmg.total : 0) + (template.crit1 ? template.crit1.total : 0)) * 0.5)).replace("type", template.dmg1type) : null;
